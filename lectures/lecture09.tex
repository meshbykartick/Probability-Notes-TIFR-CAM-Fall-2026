...
\section{Quiz, 09 September 2026}


\begin{quiz}
\begin{enumerate}
\item Let $X$ be a non-negative integer-valued random variable with finite variance. Prove that
    \[
        \mathbb{P}(X=0) \leq \frac{\operatorname{Var}(X)}{\mathbb{E}(X^2)}.
    \]
    \item Let $X$ be a random variable with $\mathbb{E}(X)=0$ and
    $\operatorname{Var}(X)=\sigma^2$. Prove that for all $\lambda>0$,
    \[
